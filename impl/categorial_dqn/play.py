...
model(tf.random.normal(shape=(1, *env.get_state_shape()), dtype=tf.float32))
model.load_weights(str(logdir / 'model' / f'model_{config["test_episode"]}.h5'))

trainer_params = config['trainer_params']
support = tf.linspace(trainer_params['min_q_value'], 
                      trainer_params['max_q_value'], 
                      trainer_params['n_bins'])
z = tf.reshape(support, (1, 1, trainer_params['n_bins']))
